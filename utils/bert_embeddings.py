...
class MosaicBertForEmbeddingGenerationHF(PreTrainedModel):
    ...
    def forward(
        ...
        # Step 2: Assert all shapes match
        assert input_ids.shape == attention_mask.shape, \
            f"Mismatch: input_ids {input_ids.shape}, attention_mask {attention_mask.shape}"
        assert input_ids.shape == token_type_ids.shape, \
            f"Mismatch: input_ids {input_ids.shape}, token_type_ids {token_type_ids.shape}"
    ...
    def get_embeddings(
        ...
        if strategy == "mean":
            # [num_layers, batch_size, seq_len, hidden_size] -> [batch_size, hidden_size]
            sentence_representation = torch.stack(encoder_outputs_all[-num_layers:]).mean(dim=[0, 2])

            if hospital_ids_lens is not None and len(hospital_ids_lens) > 0:
                for length in hospital_ids_lens:
                    if length == 0:
                        continue  # skip empty segments to avoid tensor ambiguity
                    segment = sentence_representation[start_idx:start_idx + length]
                    batch_embeddings.append(torch.mean(segment, dim=0))
                    start_idx += length
                return torch.stack(batch_embeddings)
            ...
            # [num_layers, batch_size, seq_len, hidden_size] -> [num_layers, batch_size, hidden_size]
            ...
            if hospital_ids_lens is not None and len(hospital_ids_lens) > 0:
                for length in hospital_ids_lens:
                    if length == 0:
                        continue
                    segment = sentence_representation[:, start_idx:start_idx + length]
                    batch_embeddings.append(torch.mean(segment, dim=1))
                    start_idx += length
                return torch.stack(batch_embeddings)

            # Flatten layers and hidden for whole batch
            return sentence_representation.transpose(0, 1).reshape(sentence_representation.size(1), -1)
